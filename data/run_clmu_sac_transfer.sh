#!bin/bash
# how to run: sudo bash run_clmu_sac_transfer.sh
# activate conda env
...
do
    ...
    do
        ...
        # * Transfer the model to the new city
        python run_clmu_sac.py --nc_model "sac_models/${c}.nc" \
            --surf "clmu_input/surfdata_${cities[i]}.nc" \
            --forcing "clmu_input/${frocing_paths[i]}" \
            --case_name "${c}_to_${cities[i]}" \
