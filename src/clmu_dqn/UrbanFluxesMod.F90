...
! JJ
         ...
         call wasteheat( bounds, num_urbanl, filter_urbanl, eflx_wasteheat_roof, eflx_wasteheat_sunwall, &
                         eflx_wasteheat_shadewall, eflx_heat_from_ac_roof, eflx_heat_from_ac_sunwall,    &
                         eflx_heat_from_ac_shadewall, energyflux_inst, taf )
! JJ
         do fl = 1, num_urbanl
            l = filter_urbanl(fl)
            g = lun%gridcell(l)
...
! JJ
     real(r8), intent(in) :: taf(bounds%begl:bounds%endl) ! urban canopy air temperature
! JJ
    ! !LOCAL VARIABLES:
    integer fl, l, g
! JJ
    real(r8) :: COP_heating, COP_cooling
! JJ
    !EOP
    !----------------------------------------------------------------------- 
    ...
    do fl = 1, num_urbanl
       ...
       if      ( IsSimpleBuildTemp() )then
          ...
          if (trim(urban_hac) == urban_wasteheat_on) then
            ...
            COP_cooling = -0.14_r8 * (taf(l)-273.15_r8) + 7.31_r8 ! ref: Could residential air-source heat pumps exacerbate outdoor summer overheating and winter overcooling in UK 2050s climate scenarios?
            COP_heating = 0.07_r8 *(taf(l)-273.15_r8) + 3.20_r8  ! ref: Performance analysis of air source heat pumps in heating-dominated climates
            eflx_wasteheat(l) = (1._r8 + 1._r8/COP_cooling) * eflx_urban_ac(l) + &
                                 (1._r8/COP_heating - 1._r8) * eflx_urban_heat(l)
          else
