...
! JJ 2024/10/12 RL
    ! input data and output data of the neural network
    real(r8), dimension(12) :: input_state  ! 输入有14个维度
    !real(8), dimension(3) :: action, mean, log_prob  ! 输出有3个维度
    integer :: yr, mon, day, tod ! time components for RL state input
    integer :: hourly_index ! hourly index for RL state input
    integer :: dayofyear ! day of year for RL state input
    real(r8) :: test_input(in_dim)
    real(r8) :: q_values(out_dim)
    integer :: best_action
    real(r8) :: max_q
...
! JJ 2024/10/12 RL
    ac_set_point      => temperature_inst%ac_set_point_lun     , & ! Input:  [real(r8) (:)]  air conditioning set point (K)
    heat_set_point    => temperature_inst%heat_set_point_lun   , & ! Input:  [real(r8) (:)]  heating set point (K)
    vent_ach          => temperature_inst%vent_ach_lun         , & ! Input:  [real(r8) (:)]  ventilation air change rate (s-1)
! JJ 2024/10/12 RL
    ...
    hourly_index = int(tod / dtime) + 1 ! from 1 to 48

! JJ 2024/10/12 RL
    !call nn_cf_net_init()
    if (first_time) then

      call load_model("/p/clmuapp/model.bin")

      call load_price_ele('/p/clmuapp/london_avgprice.csv')
      write(iulog,*) "CLM_DEBUG: r8 kind value =", kind(w0(1,1))
      write(iulog,*) "CLM_DEBUG: w0(1,1) =", w0(1,1)
      ! random test
      call random_number(test_input)
      q_values = dqnpredict(test_input)
      best_action = maxloc(q_values, dim=1)
      
      write(iulog,*) "CLM_DEBUG: test input =", test_input
      write(iulog,*) "CLM_DEBUG: q_values =", q_values
      write(iulog,*) "CLM_DEBUG: best_action =", best_action
...
! JJ 2024/10/12 RL
         ...
         if (mon <= 4 .or. mon >= 11) then
          ac_set_point(l) = 273.15_r8 + 24.0_r8
          input_state(1) = heat_set_point(l) !(heat_set_point(l) - 273.15+10_r8) / (273.15_r8 + 19.0_r8 - 273.15_r8 + 10.0_r8)
          vent_ach(l) = 0.3_r8
          input_state(2) = t_building_bef(l) !(t_building_bef(l) - 273.15+20_r8) / (273.15_r8 + 40.0_r8 - 273.15_r8 + 20.0_r8)
          input_state(3) = taf(l) !(taf(l) - 273.15+20_r8) / (273.15_r8 + 40.0_r8 - 273.15_r8 + 20.0_r8)

          input_state(1) = (heat_set_point(l) -273.15_r8 -16.0_r8)/(24.0_r8-16.0_r8)
          input_state(2) = (t_building_bef(l) -270.0_r8)/(300.0_r8-270.0_r8)
          input_state(3) = (taf(l) -265.0_r8)/(306.0_r8-265.0_r8)

          ! 获取当前时间点的电价
          input_state(4) = price_grid(mon, day, hourly_index)/50.0_r8 ! 电价输入
          ! 下1个时刻的电价输入
          mon2 = mon
          day2 = day
          idx2 = hourly_index + 1
          if (idx2 > 48) then
              idx2 = 1
              day2 = day2 + 1
              if (day2 > days_in_month(mon2)) then
                  day2 = 1
                  mon2 = mon2 + 1
                  if (mon2 > 12) then
                      mon2 = 1
          ...
          input_state(5) = price_grid(mon2, day2, idx2)/50.0_r8 ! 下1个时刻电价输入
          ! 下2个时刻的电价输入
          idx2 = idx2 + 1
          if (idx2 > 48) then
              idx2 = 1
          ...
          input_state(6) = price_grid(mon2, day2, idx2)/50.0_r8 ! 下2一个时刻电价输入
          ! 下3个时刻的电价输入
          idx2 = idx2 + 1
          if (idx2 > 48) then
              idx2 = 1 
          ...
          input_state(7) = price_grid(mon2, day2, idx2)/50.0_r8 ! 下3一个时刻电价输入
          ! 下4个时刻的电价输入
          idx2 = idx2 + 1
          if (idx2 > 48) then
              idx2 = 1
          ...
          input_state(8) = price_grid(mon2, day2, idx2)/50.0_r8 ! 下4一个时刻电价输入

          ! calculate day of year
          if ((mod(yr,4) == 0) .and. (mod(yr,100) /= 0 .or. mod(yr,400) == 0)) then
              ! leap year
              doyem = 366
              dayofyear = day + int((275*mon)/9) - 30
              if (mon > 2) dayofyear = dayofyear - 1
          else
              ! non-leap year
              doyem = 365
              dayofyear = day + int((275*mon)/9) - 30
              if (mon > 2) dayofyear = dayofyear - 2
          end if


          ! time embedding
          input_state(9) = sin(2.0 * 3.1415926_r8 * real(tod) / 86400.0_r8) ! time of day sin
          input_state(10) = cos(2.0 * 3.1415926_r8 * real(tod) / 86400.0_r8) ! time of day cos
          input_state(11) = sin(2.0 * 3.1415926_r8 * real(dayofyear) / real(doyem)) ! day of year sin
          input_state(12) = cos(2.0 * 3.1415926_r8 * real(dayofyear) / real(doyem)) ! day of year cos

          
          q_values = dqnpredict(input_state)
          best_action = maxloc(q_values, dim=1)
          heat_set_point(l) = 273.15_r8 + 16.0_r8 + real(best_action -1)*1.0_r8
        
! JJ 202/10/12 RL
        else
          ac_set_point(l) = 273.15_r8 + 24.0_r8
          heat_set_point(l) = 273.15_r8 + 20.0_r8
          vent_ach(l) = 0.3_r8
...
! JJ 2024/10/12 RL
         ...
         !          0.5_r8*hcv_floori(l)
          a(5,5) =  ((ht_roof(l)*rho_dair(l)*cpair)/dtime) + &
                   ((ht_roof(l)*vent_ach(l))/3600._r8)*rho_dair(l)*cpair + &
                   0.5_r8*hcv_roofi(l) + &
                   0.5_r8*hcv_sunwi(l)*canyon_hwr(l) + &
                   0.5_r8*hcv_shdwi(l)*canyon_hwr(l) + &
                   0.5_r8*hcv_floori(l)
         ...
         !             + 0.5_r8*hcv_floori(l)*(t_floor_bef(l) - t_building_bef(l))
         result(5) = (ht_roof(l)*rho_dair(l)*cpair/dtime)*t_building_bef(l) &
                      + ((ht_roof(l)*vent_ach(l))/3600._r8)*rho_dair(l)*cpair*taf(l) &
                      + 0.5_r8*hcv_roofi(l)*(t_roof_inner_bef(l) - t_building_bef(l)) &
                      + 0.5_r8*hcv_sunwi(l)*(t_sunw_inner_bef(l) - t_building_bef(l))*canyon_hwr(l) &
                      + 0.5_r8*hcv_shdwi(l)*(t_shdw_inner_bef(l) - t_building_bef(l))*canyon_hwr(l) &
                      + 0.5_r8*hcv_floori(l)*(t_floor_bef(l) - t_building_bef(l))
...
! JJ 2024/10/12 RL
         enrgy_bal_buildair(l) = (ht_roof(l)*rho_dair(l)*cpair/dtime)*(t_building(l) - t_building_bef(l)) &
                                 !- ht_roof(l)*(vent_ach/3600._r8)*rho_dair(l)*cpair*(taf(l) - t_building(l)) &
                                  - ((ht_roof(l)*vent_ach(l))/3600._r8)*rho_dair(l)*cpair*(taf(l) - t_building(l)) &
...
! JJ 2024/10/12 RL
    ...
    do fl = 1,num_urbanl
       l = filter_urbanl(fl)
       if (urbpoi(l)) then
          t_building_max(l) = ac_set_point(l)
          t_building_min(l) = heat_set_point(l)
